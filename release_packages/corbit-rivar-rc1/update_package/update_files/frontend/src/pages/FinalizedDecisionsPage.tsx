        ...
        actions={
          <>
            {(user?.role === 'pm' || user?.role === 'admin') && decisions.filter(d => d.status === 'PROPOSED').length > 0 && (
              <Button variant="contained" size="small" color="success" onClick={handleFinalizeSelected} startIcon={<LockIcon sx={{ fontSize: 15 }} />}>
                Finalize PROPOSED ({decisions.filter(d => d.status === 'PROPOSED').length})
              </Button>
            )}
            <Button variant="outlined" size="small" startIcon={<RefreshIcon sx={{ fontSize: 15 }} />} onClick={fetchDecisions}>Refresh</Button>
          </>
        }
      />
