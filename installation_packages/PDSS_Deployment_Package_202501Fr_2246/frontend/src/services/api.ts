...
export const analyticsAPI = {
  getEVA: (projectId: number | 'all') => 
    projectId === 'all' ? api.get('/analytics/portfolio/eva') : api.get(`/analytics/eva/${projectId}`),
  getCashflowForecast: (projectId: number | 'all', monthsAhead: number = 12) => 
    projectId === 'all' 
      ? api.get('/analytics/portfolio/cashflow-forecast', { params: { months_ahead: monthsAhead } })
