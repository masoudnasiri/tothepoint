...
async def create_comprehensive_projects(db: AsyncSession):
    ...
    projects_data = [
        {
            'project_code': 'INFRA001',
            'name': 'Highway Infrastructure Project',
            'priority_weight': 9,
            'description': 'Major highway construction and infrastructure development'
        },
        {
            'project_code': 'BUILD002', 
            'name': 'Commercial Building Complex',
            'priority_weight': 8,
            'description': 'Multi-story commercial building with parking facilities'
        },
        {
            'project_code': 'RESI003',
            'name': 'Residential Housing Development',
            'priority_weight': 7,
            'description': 'Affordable housing project with community facilities'
        },
        {
            'project_code': 'INDU004',
            'name': 'Industrial Manufacturing Plant',
            'priority_weight': 6,
            'description': 'Heavy industrial facility with specialized equipment'
        },
        {
            'project_code': 'UTIL005',
            'name': 'Utilities Infrastructure Upgrade',
            'priority_weight': 5,
            'description': 'Water, sewage, and electrical infrastructure modernization'
        }
    ]
    ...
    await db.commit()
    logger.info("5 comprehensive projects created successfully")
...
async def create_comprehensive_procurement_options(db: AsyncSession):
    """Create 3 procurement options per item (150 total)"""
    ...
    for item in items:
        ...
        for i, supplier in enumerate(selected_suppliers):
            # Base cost varies by supplier (competitive pricing)
            base_cost_multiplier = random.uniform(0.85, 1.15)  # ±15% variation
            base_cost = Decimal(str(round(100 * base_cost_multiplier, 2)))  # Simplified base cost
    ...
    await db.commit()
    logger.info("150 comprehensive procurement options created successfully")


async def create_comprehensive_delivery_options(db: AsyncSession):
    """Create 2 delivery options per item with invoice forecasts"""
    items = await db.execute(select(ProjectItem))
    items = items.scalars().all()
    ...
    for item in items:
        ...
        for i in range(2):
            ...
            # Invoice amount (typically 10-20% more than procurement cost for profit margin)
            base_cost = random.uniform(80, 150)  # Simplified base cost
            invoice_amount = base_cost * item.quantity * random.uniform(1.1, 1.2)
            ...
            delivery_option = DeliveryOption(
                ...
                invoice_amount_per_unit=Decimal(str(round(invoice_amount / item.quantity, 2))),
                preference_rank=i + 1,
    ...
    await db.commit()
    logger.info("100 comprehensive delivery options created successfully")


async def create_comprehensive_budget_data(db: AsyncSession):
    """Create comprehensive budget data across the year"""
    base_date = date(2025, 1, 1)
    budget_data = []
    
    # Create monthly budget allocations (more realistic for construction projects)
    monthly_budgets = [
    ...
    for month in range(12):
        ...
        budget_amount = Decimal(str(monthly_budgets[month]))
    ...
    await db.commit()
    logger.info("Comprehensive budget data created successfully")
