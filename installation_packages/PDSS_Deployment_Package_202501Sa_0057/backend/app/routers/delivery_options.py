...
async def get_delivery_options_by_item_code(
    ...
    """Get all delivery options for a specific item code (across all projects)"""
    # First, get all project items with this item_code
    result = await db.execute(
        select(ProjectItem).where(ProjectItem.item_code == item_code)
    )
