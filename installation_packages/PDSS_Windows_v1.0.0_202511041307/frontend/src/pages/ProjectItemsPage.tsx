  ...
  const renderFormFields = () => (
    <>
      ...
      <Autocomplete
        fullWidth
        options={masterItems}
        getOptionLabel={(option) => `${option.item_code} - ${option.company} - ${option.item_name}${option.model ? ` (${option.model})` : ''}`}
        ...
        filterOptions={(options, { inputValue }) => {
          ...
          return options.filter((option) => {
            const code = (option.item_code || '').toLowerCase();
            const name = (option.item_name || '').toLowerCase();
            const company = (option.company || '').toLowerCase();
            const model = (option.model || '').toLowerCase();
            const category = ((option as any).category || '').toLowerCase();
            return (
        ...
        renderInput={(params) => (
          <TextField
            ...
            margin="dense"
            placeholder={masterItems.length === 0 ? t('projectItems.noItemsInCatalog') : t('projectItems.searchPlaceholder')}
          />
        )}
          ...
          <Box component="li" {...props} key={option.id}>
            <Box>
              ...
              <Typography variant="caption" color="text.secondary">
                {option.company} - {option.item_name} {option.model && `(${option.model})`}
              </Typography>
            </Box>
          </Box>
        )}
        noOptionsText={masterItems.length === 0 ? t('projectItems.noItemsInCatalog') : t('projectItems.noItemsFound')}
        sx={{ mb: 2 }}
      />
        ...
        <List dense>
          {formData.delivery_options.map((dateStr, index) => (
            <ListItem key={index} sx={{ bgcolor: 'background.paper', mb: 0.5, borderRadius: 1 }}>
              <ListItemText
                primary={formatDisplayDate(dateStr)}
                ...
                <TableCell align="center">
                  <Box>
                    {item.delivery_options && item.delivery_options.length > 0 ? (
                      <>
                        <Typography variant="body2">
                          {formatDisplayDate(item.delivery_options[0])}
                        </Typography>
                    ...
                    onClick={() => {
                      ...
                      if ((item as any).master_item_id) {
                        ...
                        itemsMasterAPI.get(mid).then(response => {
                          ...
                          console.error('Failed to load master item', err);
                          setSelectedMasterItem(null);
                        });
              ...
              <Box sx={{ display: 'grid', gap: 2 }}>
                ...
                <Paper elevation={1} sx={{ p: 2, bgcolor: 'grey.50' }}>
                  <Typography variant="caption" color="textSecondary">
                    {t('projectItems.created')}: {formatDisplayDateTime(selectedItem.created_at)}
                  </Typography>
                  {selectedItem.updated_at && (
                    <>
                      <br />
                      <Typography variant="caption" color="textSecondary">
                        {t('projectItems.updated')}: {formatDisplayDateTime(selectedItem.updated_at)}
                      </Typography>
                    </>
                  )}
                </Paper>
