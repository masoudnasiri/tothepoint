...
export const CoverageSummaryModal: React.FC<CoverageSummaryModalProps> = ({
  ...
  const fetchCoverageSummary = async () => {
    ...
    try {
      ...
      let data = response.data as CoverageData[];
      ...
      const fullyCovered = data.filter((item) => item.coverage_percentage === 100).length;
      const partiallyCovered = data.filter(
        (item) => item.coverage_percentage > 0 && item.coverage_percentage < 100
      ).length;
      const uncovered = data.filter((item) => item.coverage_percentage === 0).length;
      const overallCoverage =
        totalItems > 0
          ? data.reduce((sum, item) => sum + item.coverage_percentage, 0) / totalItems
          : 0;
                  ...
                  {coverageData.length === 0 ? (
                    ...
                    coverageData.map((item) => (
                      <TableRow key={item.project_item_id}>
                        ...
                        <TableCell align="right">{item.required_quantity}</TableCell>
                        <TableCell align="right">{item.covered_quantity}</TableCell>
                        <TableCell align="right">
                          <Chip
                            label={item.remaining_quantity}
                            size="small"
                            color={item.remaining_quantity > 0 ? 'warning' : 'success'}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <Tooltip title={getCoverageLabel(item.coverage_percentage)}>
                            <Box>
                              <Chip
                                label={`${Math.round(item.coverage_percentage)}%`}
                                size="small"
                        ...
                        <TableCell align="center">
                          {item.remaining_quantity > 0 && onCreateForRemaining && (
                            <Button
                              size="small"
                              variant="outlined"
                              onClick={() => {
                                if (onCreateForRemaining) {
                                  onCreateForRemaining({
                                    ...
                                    main_item_remaining: item.remaining_quantity,
                                    subitem_remaining: item.subitems
              ...
              return (
                <Paper key={`subitems-${item.project_item_id}`} sx={{ p: 2, mt: 2 }}>
                  <Typography variant="subtitle2" gutterBottom>
                    {item.item_code} - {t('procurement.subItems') || 'Sub-Items'}
                  </Typography>
                        ...
                        >
                          ...
                          <Box sx={{ mt: 0.5 }}>
                            <Chip
                              ...
                              size="small"
                              color={getCoverageColor(subitem.coverage_percentage)}
                            />
