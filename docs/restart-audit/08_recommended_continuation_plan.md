...
- Sprint 5C-R4-Fix-2 RBAC Labels & Master Data Access Leakage (`PASS WITH MINOR ISSUES`, `67_*`): branch `restart/sprint5c-r4-fix2-rbac-labels-masterdata-leakage`; Access Control badges show Enforced not Pilot; master-data nav uses effective RBAC grants; Payment Methods frontend gating; payment-methods/cost-components backend enforcement gap documented
- Next planned work: Sprint 5D Procurement Assignment Backend (after PO sign-off on Payment Methods / Cost Components backend RBAC gap)
