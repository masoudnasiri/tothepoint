...
- Sprint 5C-R1 RBAC UX + Master Data Permission Pilot (`PASS WITH MINOR ISSUES`, `60_*`): branch `restart/sprint5c-r1-rbac-ux-masterdata-permission-pilot` @ `8e0770a`; unified Users & Access Control nav, user RBAC role assignment, permission matrix UX, Items Master/Suppliers pilot enforcement (backend + frontend); runtime closure deferred
...
- Sprint 5C-R4 Full RBAC Inventory & E2E Access Audit (`PASS WITH MINOR ISSUES`, `64_*` / `65_*`): branch `restart/sprint5c-r4-full-rbac-inventory-e2e-audit` @ `7801c5e`; code-driven inventory, users.* backend enforcement, badge fix; demo deploy deferred in sprint
...
- Sprint 5E Procurement Assignment Frontend and Scoped UX (`PASS WITH MINOR ISSUES`, `70_*`): branch `restart/sprint5e-procurement-assignment-frontend-scoped-ux`; assignment tab on Project Items, My Assignments on Procurement page, API client, i18n EN/FA; no backend scope enforcement
- Sprint 5E-R1 Procurement Assignment UX Relocation (`PASS WITH MINOR ISSUES`, `71_*`): branch `restart/sprint5e-r1-procurement-assignment-ux-relocation`; primary assignment management moved to Procurement module tab; Project Items read-only summary + link only
- Sprint 5E-R2 Procurement Assignment Workbench View Modes (`PASS WITH MINOR ISSUES`, `72_*`): branch `restart/sprint5e-r2-procurement-assignment-workbench-view-modes`; View by Project/Item, remove selected, complete de-emphasized, finalization helper text
- Sprint 5E-R2-Fix Secure Assigned Item Visibility (`PASS WITH MINOR ISSUES`, `73_*`): branch `restart/sprint5e-r2-fix-secure-assigned-item-visibility`; safe assigned-items dialog + sanitized API + Project Items route guard; blocks procurement view-only users from full Project Items page
- Sprint 5E-R2-Fix-R1 Secure Assigned Item Runtime Closure (`PASS WITH MINOR ISSUES`, `74_*`): branch `restart/sprint5e-r2-fix-r1-secure-item-runtime-closure`; deployed exact `3057651` to `/opt/rivar-demo`; assigned-items endpoints live in OpenAPI; runtime guard/security smoke and verify.sh PASS; `/items/finalized` remains 5F follow-up
- Sprint 5E-R3 Bulk Remove Procurement Assignments (`PASS`, `75_*`): branch `restart/sprint5e-r3-bulk-remove-procurement-assignments` @ `7176f27`; assignable vs removable selection clarity, project-level + item-level bulk remove/cancel flow, partial-failure summary, runtime deploy to `/opt/rivar-demo`, verify.sh and targeted runtime smoke PASS
- Sprint 5E-R3-Fix Payment Methods Navigation + Provenance Cleanup (`PASS`, `76_*`): branch `restart/sprint5e-r3-fix-payment-methods-navigation`; restored Payment Methods nav/route for authorized users, added route guard and granular payment-method action gating, preserved unauthorized 403 behavior, synchronized root/docs deployment markers on `/opt/rivar-demo`, and revalidated procurement assignment + safe assigned-item regressions
- Sprint 5E-R3-Fix Compile Hotfix - ProcurementPage Permission Helper Compile Fix (`PASS`, `76_*` hotfix section): same branch; restored missing `isAssignedOnlyProcurementScopeUser` / `isGlobalProcurementScopeUser` exports in frontend permissions helper, resolved runtime frontend compile blocker, rebuilt `/opt/rivar-demo` frontend, verify.sh PASS, and rechecked Payment Methods + procurement assignment/safe-item regressions
- Sprint 5E-R4 Finalized-Only Procurement Assignment Item Visibility (`PASS WITH MINOR ISSUES`, `78_*`): branch `restart/sprint5e-r4-finalized-only-assignment-items`; enforced finalized-only item visibility/selection/assignment in procurement assignment surfaces, preserved project-level assignment before finalization, added backend non-finalized item-level rejection + safe assigned-items finalized filtering, revalidated bulk remove + Payment Methods + Package Wizard/verify runtime checks on `/opt/rivar-demo`
- Sprint 5F Procurement Assignment Scope Enforcement and Procurement Workflow Filtering (`PASS WITH MINOR ISSUES`, `79_*`): branch `restart/sprint5f-procurement-assignment-scope-enforcement`; added active-assignment scope enforcement for finalized procurement workflow endpoints (`/items/finalized`, procurement options, package/project package coverage flows), assigned-only frontend scoped messaging and global-action gating, 5E-R4 carry-over cleanup (`78` provenance + readiness-id hardcode removal), focused backend/frontend test pass; scoped bulk rollback for assigned-only deferred as follow-up
- Sprint 5F-Fix Procurement Assignment Table Display and Sprint Closure (`PASS WITH MINOR ISSUES`, `80_*`): branch `restart/sprint5f-fix-assignment-table-display-closure`; separated assignment table columns (item, assigned users, status, actions) for clearer RTL/LTR readability, added selection tooltips/labels, and updated focused workbench regression assertions; remote deployment step requires valid SSH credentials in this environment to finalize live marker update.
- Sprint 5F-Fix-2 Runtime Deployment and Live UI Closure (`PASS WITH MINOR ISSUES`, `81_*`): branch `restart/sprint5f-fix2-runtime-ui-closure`; verified stale runtime marker (`c1e90b2...`) on `/opt/rivar-demo`, deployed exact `ab7f713...`, rebuilt/recreated frontend container, updated root/docs deployment markers to `5F-Fix-2`, confirmed bundle ETag/content change, reran runtime smoke and `verify.sh` PASS.
- Next planned work: Sprint 5G Procurement-to-Optimization Decision Handoff UX (or 5F-Fix-3 only if user still reproduces table issue after hard refresh).
