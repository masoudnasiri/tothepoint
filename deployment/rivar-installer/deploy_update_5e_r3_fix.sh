#!/usr/bin/env bash
# Sprint 5E-R3-Fix: restore Payment Methods navigation and sync deployment markers.
set -euo pipefail
...
DEPLOY_BRANCH="${2:-restart/sprint5e-r3-fix-payment-methods-navigation}"
DEPLOYED_AT="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

echo "=== Rivar demo update deploy (5E-R3-Fix) ==="
...
# Keep docs marker mirrored to prevent provenance mismatch during transition.
