#!/usr/bin/env bash
# Sprint 5C-R4-Fix: rebuild /opt/rivar-demo after syncing source (preserve .env + DB volume).
set -euo pipefail
...
DEPLOY_BRANCH="${2:-restart/sprint5c-r4-full-rbac-inventory-e2e-audit}"

echo "=== Rivar demo update deploy ==="
...
cat > docs/restart-audit/DEPLOYED_COMMIT.txt <<EOF
