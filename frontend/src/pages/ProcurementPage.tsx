          ...
          >
            ...
            <AccordionDetails>
              ...
              <PackageList
                ...
                onEdit={async (packageId) => {
                  try {
                    // Fetch package details
                    const packageResponse = await packagesAPI.get(packageId);
                    const packageData = packageResponse.data;
                    
                    // Fetch package subitems
                    const subitemsResponse = await packagesAPI.get(packageId);
                    const subitems = subitemsResponse.data?.subitems || [];
                    
                    // Fetch procurement options for this package to get pricing info
                    ...
                    const existingPackagesResponse = await packagesAPI.listByProjectItem(itemDetails.project_item_id, true);
                    setWizardExistingPackages(existingPackagesResponse.data || []);
                    
                    // Prepare initial data for wizard
                    const subitemQuantities: Record<number, number> = {};
                    subitems.forEach((si: any) => {
                      // Find the sub_item_id from the requirement
                      const req = subItemRequirements.find(r => r.item_subitem_id === si.project_item_subitem_id);
                      if (req) {
                        subitemQuantities[req.sub_item_id] = si.quantity_covered || 0;
                      }
                    });
                    
                    setWizardInitialData({
                      package_name: packageData.package_name || '',
                      supplier_id: packageData.supplier_id || null,
                      package_type: packageData.package_type || 'CUSTOM',
                      description: packageData.description || '',
                      main_item_quantity: packageData.main_item_quantity || 0,
                      subitem_quantities: subitemQuantities,
                      base_cost: packageOption?.base_cost || packageOption?.cost_amount || 0,
                      currency_id: packageOption?.currency_id || null,
                      shipping_cost: packageOption?.shipping_cost || 0,
                      delivery_option_id: packageOption?.delivery_option_id || null,
                      lomc_lead_time: packageOption?.lomc_lead_time || 0,
                      ...
                      payment_terms: packageOption?.payment_terms || {
                        type: 'cash',
                        discount_percent: 0,
                      },
                    ...
                    setError(formatApiError(err, t('procurement.failedToLoadPackage') || 'Failed to load package for editing'));
                  }
                }}
                ...
                <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
                  <Button
                    ...
                    onClick={async () => {
                      // Open package wizard
                      try {
                        // Fetch sub-item requirements using the correct API endpoint
                        const subItemsResponse = await itemsAPI.listProjectItemSubItems(itemDetails.project_item_id);
                        const subItemRequirements: SubItemRequirement[] = subItemsResponse.data.map((sub: any) => ({
                          sub_item_id: sub.sub_item?.id || sub.id, // The sub-item master ID
                          item_subitem_id: sub.id, // The project_item_subitems table ID (this is what we need for API)
                          name: sub.sub_item?.name || `Sub-item ${sub.sub_item?.id || sub.id}`,
                          part_number: sub.sub_item?.part_number,
                          required_quantity: sub.quantity || 0,
                        }));
        ...
        <CoverageSummaryModal
          ...
          onCreateForRemaining={(remainingDemand) => {
            ...
            if (itemDetails) {
              ...
              // Note: When creating from remaining demand, we need to fetch the item_subitem_id
              // For now, we'll set it to null and the wizard will need to handle it
              setWizardSubItemRequirements(
                remainingDemand.subitem_remaining.map((si: any) => ({
                  sub_item_id: si.sub_item_id,
                  item_subitem_id: si.item_subitem_id, // May not be available, will need to fetch
      ...
      {packageWizardOpen && wizardProjectItemId && (
        <PackageWizard
          ...
          onPackageCreated={async (packageId) => {
            // Refresh data after package creation/update
            setPackageRefreshTrigger(prev => prev + 1);
            fetchData();
            setEditingPackageId(null);
            setWizardInitialData(null);
