    ...
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" mt={1}>
            Configure optimization factor priorities based on available project items and procurement options. All factors default to weight 1.
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      <Alert severity="info" sx={{ mb: 2 }}>
        <Typography variant="body2">
          <strong>Available Factors:</strong> {availableFactors.length} factors discovered from your project items and procurement options.
          {availableFactors.filter(factor => !weights.some(w => w.factor_name === factor)).length > 0 && (
            <span> {availableFactors.filter(factor => !weights.some(w => w.factor_name === factor)).length} factors not yet configured (will use default weight 1).</span>
          )}
        </Typography>
      </Alert>
